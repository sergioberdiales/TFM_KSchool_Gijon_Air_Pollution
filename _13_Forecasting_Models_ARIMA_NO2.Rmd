...
residual_distribution_base_model_graph <- ggplot(data = df, aes(x = y_pred, y = (y_pred - y_test))) + 
                  geom_point() +
...
residual_histogram_base_model_graph <- ggplot(data = df, aes(x = y_pred - y_test)) +
                        geom_histogram() +
                        theme_minimal()
```
...
residuals_dates <- bind_cols(date_time, y_pred, y_test) %>%
                   mutate(residuals = y_pred - y_test) %>%
...
residual_distribution_arima_3m_graph <- ggplot(data = df, aes(x = y_pred, y = (y_pred - y_test))) + 
                  geom_point() +
                  geom_hline(yintercept=0) +
                  geom_point(alpha = 0.5) +
                        theme_minimal()

residual_histogram_arima_3m_graph <- ggplot(data = df, aes(x = y_pred - y_test)) +
                        geom_histogram() +
                        theme_minimal()
...
monthly_mae_comparison <- monthly_mae_comparison %>% gather(model, mae, 2:3)

monthly_mae_base_model_vs_arima3m_graph <- ggplot(data = monthly_mae_comparison, 
...
residual_distribution_arima_3y_graph <- ggplot(data = df, aes(x = y_pred, y = (y_pred - y_test))) + 
                  geom_point() +
                  geom_hline(yintercept=0) +
                  geom_point(alpha = 0.5) +
                        theme_minimal()
...
residual_distribution_arima_9y_graph <- ggplot(data = df, aes(x = y_pred, y = (y_pred - y_test))) + 
                  geom_point() +
                  geom_hline(yintercept=0) +
                  geom_point(alpha = 0.5) +
                        theme_minimal()
...
As with the PM10 we are going to create a Forecasting Base Model to use as reference.

And as before, we will take as prediction the value from the previous hour, forecasting just one hour ahead.
...
test_201701_201709_2 <- constitucion_data %>% filter(date_time_utc >= '2017-01-01 00:00:00',
                                       date_time_utc < '2017-10-01 00:00:00') %>%
                                       select(NO2) %>%
                                       mutate(NO2 = replace_na(NO2, mean(NO2, na.rm = TRUE))) # We replace the nas (36) by the mean.


base_model <- test_201701_201709_2 %>% mutate(y_pred = lag(NO2, 1)) %>% # We create the column y_pred with the lagged value of NO2 (one hour).
                                     rename(y_test = NO2) %>%         # We change the name of the NO2 column to y_test
                                     na.omit()                         # We remove the observations with nas (just the first row)
...
Pending: comment the results. better than the PM10 model. compare them graphically (errors and hourly curves)

### NO2 ARIMA models

NO2_model_arima_3m
```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
# NO2_model_arima_3m = auto.arima(train_201610_201612,seasonal=TRUE,trace=TRUE) 
# saveRDS(NO2_model_arima_3m, "data_rds/NO2_model_arima_3m.rds")
# summary(NO2_model_arima_3m)
```

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
NO2_model_arima_3m <- readRDS("data_rds/NO2_model_arima_3m.rds")
summary(NO2_model_arima_3m)
...
y_pred <- forecast(test_201701_201709, model = NO2_model_arima_3m, h=1)$fitted # We call the forecast function passing the testing data through the ARIMA model fitted with the training data and we extract the fitted values.
...
Pending: 
Very similar results to the base model.
Better R-squared than the PM10 model.
But worse errors scores. Different units. We can't compare.
More variability. 
...
NO2_model_arima_3y
```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
# NO2_model_arima_3y = auto.arima(train_201401_201612,seasonal=TRUE,trace=TRUE) 
# saveRDS(NO2_model_arima_3y, "data_rds/NO2_model_arima_3y.rds")
# summary(NO2_model_arima_3y)
```

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
NO2_model_arima_3y <- readRDS("data_rds/NO2_model_arima_3y.rds")
summary(NO2_model_arima_3y)
...
y_pred <- forecast(test_201701_201709, model = NO2_model_arima_3y, h=1)$fitted # We call the forecast function passing the testing data through the ARIMA model fitted with the training data and we extract the fitted values.
...
NO2_model_arima_3y
```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
#NO2_model_arima_9y = auto.arima(train_200901_201612,seasonal=TRUE,trace=TRUE) 
#saveRDS(NO2_model_arima_9y, "data_rds/NO2_model_arima_9y.rds")
#summary(NO2_model_arima_9y)
```


```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
NO2_model_arima_9y <- readRDS("data_rds/NO2_model_arima_9y.rds")
summary(NO2_model_arima_9y)
```
...
y_pred <- forecast(test_201701_201709, model = NO2_model_arima_9y, h=1)$fitted # We call the forecast function passing the testing data through the ARIMA model fitted with the training data and we extract the fitted values.
...
With the 9 years training period the model is not clearly better than with 3 years.  
