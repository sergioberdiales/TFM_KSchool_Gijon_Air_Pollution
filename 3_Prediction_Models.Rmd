...
train_lite <- constitucion_data %>% filter(date_time_utc >= '2016-10-01 00:00:00',
                                       date_time_utc <= '2016-12-31 23:00:00') %>%
                                       select(PM10) %>%
                                       mutate(PM10 = replace_na(PM10, mean(PM10, na.rm = TRUE))) %>%
                                       rename(PM10_0 = PM10) %>%
...
saveRDS(h6, "data_rds_files/arima_model_0_test_fitted_values_h6.rds") # We save the final object as a rds file
```
```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
fitted_values_h6 <- readRDS("data_rds_files/arima_model_0_test_fitted_values_h6.rds")
```
...
We repeat ...
```
model_arima_1 = auto.arima(train_lite,seasonal=TRUE,trace=TRUE) # 
saveRDS(model_arima_1, "data_rds_files/model_arima_1.rds")
```

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
model_arima_1 <- readRDS("data_rds_files/model_arima_1.rds")
summary(model_arima_1)
```

The model selected by the auto.arima function is very similar to the former model

train_lite:          ARIMA(2,1,4)(0,0,2)[24]
...
arima.test_lite <- Arima(test_lite, model=model_arima_1)
accuracy(arima.test)
```

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
h1_lite <- fitted(arima.test_lite, h = 1) 
h2_lite <- fitted(arima.test_lite, h = 2)
```



```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
h6_lite <- fitted(arima.test_lite, h = 6) 
```
...
Using the train_lite
```
NO2_model_arima_1 = auto.arima(train_lite,seasonal=TRUE,trace=TRUE) 
saveRDS(NO2_model_arima_1, "data_rds_files/NO2_model_arima_1.rds")
```

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}
NO2_model_arima_1 <- readRDS("data_rds_files/NO2_model_arima_1.rds")
...
Pending: 
  ...
  - Comentar graficos de distribuciones de errores. A;adir linea de tendencia
  ...
  - Hacer una mejor imputacion de nas. Ahora mismo estoy utilizando la media.
      ...
      - Utilizar un periodo de train menor. HECHO
