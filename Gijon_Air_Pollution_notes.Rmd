...
kable(head(data_completion, 10))

```

We are going to check the data completion by station:

constitución: desde el anho 2000 tiene registrados datos de SO2, NO, NO2, CO, PM10, 03, dd, vv, TMP,  HR, PRB, HS y LL, 
A partir del anho 2007 tambien existen datos de BEN, TOL y MXIL (en 2006 sólo el 0,01% del año). Y a partir de 2009 se empieza a medir las particulas PM25 (en 2008 sólo el 0,02% del año).
...
```{r warning = FALSE, message = FALSE}
montevil_data <- data_completion %>% filter(station_alias == 'Montevil')
kable(montevil_data)
```

barbara: solo datos de 2016 y 2017 de las variables NO, NO2, CO, PM10, 03 y PM25
```{r warning = FALSE, message = FALSE}
barbara_data <- data_completion %>% filter(station_alias == 'Santa Bárbara')
kable(barbara_data)
```
