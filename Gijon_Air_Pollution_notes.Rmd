...
We create a file list, data_files, with the csv files previously downloaded. 
...
We take a look to the basic attributes of the dataset variables with the glimpse and the summary functions:
```{r warning= FALSE, message= FALSE}
glimpse(air_data_0)
...
air_data_2$week_day <- wday(air_data_2$date_time_utc)
...
avgs <- air_data_2 %>% select(station_alias, date_time_utc, PM10, PM25, SO2, NO2, NO, O3, BEN, CO, MXIL, TOL) %>%
...
avgs_largo <- gather(avgs, contaminante, value, 3:length(avgs))

# lo representamos en un grid

ggplot(avgs_largo, aes(x = year, y = value)) + 
  geom_line() + 
  facet_grid(contaminante~station_alias,scales="free_y") +
   theme(axis.text = element_text(size = 6))


# Me quedo solo con las estaciones urbanas

avgs_largo_urb <- avgs_largo %>% filter(station_alias != 'Montevil' , 
                                         station_alias != 'Santa Bárbara' )

ggplot(avgs_largo_urb, aes(x = year, y = value)) + 
  geom_line() + 
  facet_grid(contaminante~station_alias,scales="free_y") +
   theme(axis.text = element_text(size = 6))
...
avg_pm10 <- avgs_largo %>% filter(contaminante == 'PM10')  

ggplot(avg_pm10, aes(x = year, y = value)) + 
  geom_line() + 
  facet_wrap(~station_alias)  
...
hour_o3 <- air_data_2 %>% select(O3, hour) %>% 
                      group_by(hour) %>%
                      summarise(O3_mean = round(mean(O3, na.rm = TRUE),1))

ggplot(hour_o3, aes(hour, O3_mean)) + 
   geom_line()

hour_pm10 <- air_data_2 %>% select(PM10, hour) %>% 
                      group_by(hour) %>%
                      summarise(PM10_mean = round(mean(PM10, na.rm = TRUE),1))

ggplot(hour_pm10, aes(hour, PM10_mean)) + 
   geom_line()

hour_pm25 <- air_data_2 %>% select(PM25, hour) %>% 
                      group_by(hour) %>%
                      summarise(PM25_mean = round(mean(PM25, na.rm = TRUE),1))

ggplot(hour_pm25, aes(hour, PM25_mean)) + 
   geom_line()

#pairs(air_data_2[,6:22], pch = 19, lower.panel = NULL)
