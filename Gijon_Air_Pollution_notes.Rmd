...
There are measurements of the variables BEN, TOL and MXIL since the year 2006 (only 0.01% ). Y a partir de 2009 se empieza a medir las particulas PM25 (en 2008 sólo el 0,02% del año).
...
year_const <- year_avgs_largo %>% filter(station_alias == "Constitución", contaminante == 'PM10')
plot1 <- ggplot(year_const, aes(x = year, y = value)) + 
  geom_line()

month_const <- month_avgs_largo %>% filter(station_alias == "Constitución", contaminante == 'PM10')
plot2 <- ggplot(month_const, aes(x = month, y = value)) + 
  geom_line()

week_day_const <- week_day_avgs_largo %>% filter(station_alias == "Constitución", contaminante == 'PM10')
plot3 <- ggplot(week_day_const, aes(x = week_day, y = value)) + 
  geom_line()

hour_const <- hour_avgs_largo %>% filter(station_alias == "Constitución", contaminante == 'PM10')
...
We convert the dataframe pm10 in a time series object

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}

pm10_ts <- xts(x = pm10$PM10, order.by = pm10$date_time_utc)


```

We plot the entire series. It 
