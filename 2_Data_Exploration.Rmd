...
year_avgs_long <- gather(year_avgs, contaminante, value, 3:length(year_avgs)) %>% 
                    filter(!(station_alias == 'Constituci髇' & year == '2006' & contaminante %in% c('BEN', 'MXIL', 'TOL'))) %>% # We filter this data because is only completed in 0.01%
                    filter(!(station_alias == 'Constituci髇' & year == '2008' & contaminante == 'PM25')) # We filter this data because is only completed in 0.02%
...
We drop the Santa B谩rbara and Montevil stations. These stations have much less data and the behavior of their variables are significantly different (they are sub-urban stations). So, we take them out from the analysis for now. 

```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}

air_data_3 <- air_data_2 %>% filter(station_alias != 'Montevil' , 
                                         station_alias != 'Santa B醨bara' )
...
We create the dataset pm10 with PM10 values from the Constituci贸n Station and we execute a summary


```{r warning = FALSE, message= FALSE, out.width= '\\textwidth'}

pm10 <- air_data_3 %>% filter(station_alias == 'Constituci髇') %>%
...
ggplot(pm10_day_avg, aes(x = day, y = day_avg, , colour = day_avg)) + 
         ...
         labs(colour = "PM10 daily average (colour scale)", x = "Year", y = "PM10 daily average", title = "PM10 daily average - 2000-2017 evolution (Constituci贸n Station)")
...
year_const <- year_avgs_long %>% filter(station_alias == "Constituci贸n", contaminante == 'PM10')
plot1 <- ggplot(year_const, aes(x = year, y = value)) + 
  geom_line()

month_const <- month_avgs_long %>% filter(station_alias == "Constituci贸n", contaminante == 'PM10')
plot2 <- ggplot(month_const, aes(x = month, y = value)) + 
  geom_line()

week_day_const <- week_day_avgs_long %>% filter(station_alias == "Constituci贸n", contaminante == 'PM10')
plot3 <- ggplot(week_day_const, aes(x = week_day, y = value)) + 
  geom_line()

hour_const <- hour_avgs_long %>% filter(station_alias == "Constituci贸n", contaminante == 'PM10')
