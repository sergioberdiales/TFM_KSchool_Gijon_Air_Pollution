...
I downloaded 18 csv files with air pollution and weather data of Gij贸n from years 2000 to 2017. I saved them in the "data" folder.
...
air_data_1 <- air_data_0 %>% rename(station = `Estaci髇`,
                                    station_name = `T韙ulo`,
...
alias_dict <- data.frame(
      ...
      station_alias = c("Constituci贸n", "Argentina", "H. Felgueroso", "Castilla", "Montevil", "Santa B谩rbara")
)
```
...
Constituci贸n: There is data registered from the variables SO2, NO, NO2, CO, PM10, 03, dd, vv, TMP,  HR, PRB, HS and LL since the year 2000.
...
```{r warning = FALSE, message = FALSE}
constitucion_data <- data_completeness %>% filter(station_alias == 'Constituci贸n')
...
Santa B谩rbara: Data since the year 2016. Variables: NO, NO2, CO, PM10, 03 and PM25
```{r warning = FALSE, message = FALSE}
barbara_data <- data_completeness %>% filter(station_alias == 'Santa B谩rbara')
kable(barbara_data)
```
...
# Laboral dates
http://www.calendarioslaborales.com
```{r warning = FALSE, message = FALSE}
...
air_data_2 <- air_data_2 %>%  mutate(lab = ifelse((week_day < 6 & !is.na(holiday_type)) |
                                                    (week_day >=6), "no_lab", "lab")) %>%
                                                     mutate(lab=replace(lab, date < '2014-01-01', NA))



                                            
table(air_data_2$holiday_type, air_data_2$lab)
table(air_data_2$week_day, air_data_2$lab)
...
air_data_2 <- read_rds("data_rds_files/air_data_2.rds")
...
saveRDS(air_data_2, file = "data_rds_files/air_data_2.rds")
...
saveRDS(co_data, file = "data_rds_files/co_data.rds")
...
saveRDS(so2_data, file = "data_rds_files/so2_data.rds")
...
saveRDS(so2_hourly_limit, file = "data_rds_files/so2_hourly_limit.rds")
...
saveRDS(so2_hourly_limit, file = "data_rds_files/so2_daily_limit.rds")
write_csv(so2_hourly_limit, "data_final_csvs/so2_daily_limit.csv")
